...
constexpr int HEIGHT = 40; // default: 24
...
std::string debuginfo(const double& xmin, const double& xmax, const double& ymin, const double& ymax, const int& maxIter) {
    std::stringstream ss;
...
void draw(const double xmin, const double xmax, const double ymin, const double ymax, const int maxIter) {
    ...
    std::string debugInfo = debuginfo(xmin, xmax, ymin, ymax, maxIter);
    std::cout << "\x1B[" << HEIGHT - 1500 << ";" << WIDTH - debugInfo.length() << "H\x1B[37m" << debugInfo << "\x1B[0m";
...
int main() {
    ...
    wchar_t title[] = L"PhysY 0.1.0 Debug\t\t\t\t\t\t!!! UNREGISTERED FREE HYPERCAM 2 !!!";
    wchar_t aa[1024]{};
